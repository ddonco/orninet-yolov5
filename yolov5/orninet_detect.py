...
class Detector():
    ...
    def detect(self, save_img=False):
        source, weights, view_img, save_txt, imgsz, post_results, post_url, target, enable_print, output, save_delay = \
            self.source, self.weights, self.view_img, self.save_txt, self.img_size, self.post_results, self.post_url, self.target, self.enable_print, self.output, self.save_delay
        ...
        for path, img, im0s, vid_cap, timestamp in dataset:
            ...
            dataset.shutdown_signal(signal)
...
if __name__ == '__main__':
    ...
    det = Detector()
    det.device = '0'
    det.weights = '/home/dd/source/orninet-yolov5/yolov5/weights/yolov5s.pt'
    det.source =  '0' # './inference/images/birds.jpg'
    ...
    det.view_img = False
    det.save_txt = False
    det.enable_print = False
    det.post_results = True
    det.post_url = 'http://localhost:5000/api/post-detection'
    det.log = '/home/dd/source/orninet-yolov5/yolov5/orninet.log'

    with torch.no_grad():
        if det.update:  # update all models (to fix SourceChangeWarning)
            for det.weights in ['yolov5s.pt', 'yolov5m.pt', 'yolov5l.pt', 'yolov5x.pt']:
                det.detect()
                strip_optimizer(det.weights)
        else:
            det.detect()
